...
export default function DashboardPage() {
  ...
  const availableSchools = useMemo(
    ...
    [schools, filters.district, filters.sector]
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (filters.district && !filters.schoolCode && !filters.sector) {
        ...
        } else if (filters.sector && !filters.schoolCode) {
          const sectorSchools = schools.filter((school) => school.sector === filters.sector);
          const statsPromises = sectorSchools.map(async (school) => {
            const response = await fetch(`http://localhost:3001/evaluations/teacher-evaluation-stats?schoolCode=${school.schoolCode}`);
            const data = await response.json();
            return {
              schoolCode: school.schoolCode,
  ...
  // Aggregate evaluations by district, sector, or school and role
  const aggregatedEvaluationData = useMemo(() => {
    if (!filters.district) {
      ...
      evaluations
        .filter((evalItem) => {
          ...
          return (
            ...
            evalItem.school.district === filters.district &&
            evalItem.school.sector === filters.sector
          );
        })
        .forEach((evalItem) => {
          const sector = evalItem.school.sector;
          const role = evalItem.evaluationType.name.toLowerCase();
          if (!result[sector]) {
            result[sector] = {
              headteacher: { responses: {}, teacherCodes: new Set(), sectorName: sector },
              dos: { responses: {}, teacherCodes: new Set(), sectorName: sector },
              dod: { responses: {}, teacherCodes: new Set(), sectorName: sector },
            };
          }
          result[sector][role].teacherCodes.add(evalItem.teacherCode);
          evalItem.responses.forEach((response, index) => {
            const question = `q${index + 1}`;
            result[sector][role].responses[question] = result[sector][role].responses[question] || [];
            result[sector][role].responses[question].push(response.rating);
          });
        });

      // Average the ratings for each question
      Object.keys(result).forEach((sector) => {
        Object.keys(result[sector]).forEach((role) => {
          Object.keys(result[sector][role].responses).forEach((question) => {
            const ratings = result[sector][role].responses[question];
            result[sector][role].responses[question] = ratings.length > 0
      ...
      evaluations
        .filter((evalItem) => {
          ...
          return (
            evalMonth === currentMonth &&
            evalItem.school.district === filters.district &&
            evalItem.school.schoolCode === filters.schoolCode
          );
        })
        .forEach((evalItem) => {
      ...
      // Aggregate by district and role
      const result: {
        [district: string]: {
          [role: string]: {
            responses: { [question: string]: number[] };
      ...
      evaluations
        .filter((evalItem) => {
          const evalMonth = new Date(evalItem.date).toISOString().slice(0, 7);
          return evalMonth === currentMonth && evalItem.school.district === filters.district;
        })
        .forEach((evalItem) => {
          const district = evalItem.school.district;
          const role = evalItem.evaluationType.name.toLowerCase();
          if (!result[district]) {
            result[district] = {
              headteacher: { responses: {}, teacherCodes: new Set(), districtName: district },
              dos: { responses: {}, teacherCodes: new Set(), districtName: district },
              dod: { responses: {}, teacherCodes: new Set(), districtName: district },
            };
          }
          result[district][role].teacherCodes.add(evalItem.teacherCode);
          evalItem.responses.forEach((response, index) => {
            const question = `q${index + 1}`;
            result[district][role].responses[question] = result[district][role].responses[question] || [];
            result[district][role].responses[question].push(response.rating);
          });
        });

      // Average the ratings for each question
      Object.keys(result).forEach((district) => {
        Object.keys(result[district]).forEach((role) => {
          Object.keys(result[district][role].responses).forEach((question) => {
            const ratings = result[district][role].responses[question];
            result[district][role].responses[question] = ratings.length > 0
              ? ratings.reduce((a, b) => a + b, 0) / ratings.length
              : 0;
          });
        });
  ...
  }, [evaluations, filters.district, filters.sector, filters.schoolCode, filters.month]);
  ...
  const calculateMetrics = () => {
    if (!filters.district) {
      ...
      // Metrics for specific district, sector, or school
      const metrics: {
        [key: string]: {
          [role: string]: {
            totalScore: number;
            ...
            name: string; // districtName, sectorName, or schoolName
            sector?: string; // Only for school-level
          };
        };
      } = {};
      let totalTeachers = 0;
      ...
      Object.keys(aggregatedEvaluationData).forEach((key) => {
        const data = aggregatedEvaluationData[key];
        const isSchool = filters.schoolCode && filters.schoolCode !== "all-schools";
        const isSector = filters.sector && !filters.schoolCode;
        const stats = teacherStats.filter((stat) => 
          isSchool ? stat.schoolCode === key :
          isSector ? stat.sector === key :
          stat.sector && schools.find(s => s.schoolCode === stat.schoolCode)?.district === key
        );
        totalTeachers += stats.reduce((sum, stat) => sum + (stat.totalTeachers || 0), 0);
        teachersEvaluated += stats.reduce((sum, stat) => sum + (stat.teachersEvaluated || 0), 0);

        metrics[key] = {};
        ["headteacher", "dos", "dod"].forEach((role) => {
          const roleData = data[role];
          if (roleData && Object.keys(roleData.responses).length > 0) {
            const scores = Object.values(roleData.responses) as number[];
            const total = scores.reduce((sum, score) => sum + score, 0);
            const average = scores.length > 0 ? total / scores.length : 0;
            const teacherCount = roleData.teacherCodes.size;
            const evaluationPercentage = stats.length > 0
              ? (teacherCount / stats.reduce((sum, stat) => sum + (stat.totalTeachers || 0), 0)) * 100
              : 0;
            ...
            metrics[key][role] = {
              ...
              name: isSchool ? roleData.schoolName : isSector ? roleData.sectorName : roleData.districtName,
              sector: isSchool ? roleData.sector : undefined,
            };
          }
        });
      });
      ...
      Object.values(metrics).forEach((keyMetrics) => {
        ["headteacher", "dos", "dod"].forEach((role) => {
          if (keyMetrics[role] && keyMetrics[role].responseCount > 0) {
            roleAverages[role].total += keyMetrics[role].percentage;
            roleAverages[role].count += 1;
          }
        });
      });
      ...
      const districtMetrics = {
        averageScore: Object.values(metrics)
          .flatMap((key) => Object.values(key).map((role) => role.averageScore))
          .reduce((sum, score) => sum + (score || 0), 0) /
          Object.values(metrics).flatMap((key) => Object.keys(key)).length || 0,
  ...
  const overviewData = useMemo(() => {
    if (!filters.district) {
      ...
      Object.keys(metrics).forEach((key) => {
        ["headteacher", "dos", "dod"].forEach((role) => {
          const roleData = metrics[key][role];
  ...
  const exportToExcel = () => {
    if (!filters.district) {
      ...
      const evaluationData = Object.keys(metrics).flatMap((key) =>
        ["headteacher", "dos", "dod"].map((role) => {
          const roleData = metrics[key][role];
          if (!roleData || roleData.responseCount === 0) return null;
          return {
            District: filters.district,
            Sector: roleData.sector || (filters.sector || "All Sectors"),
            School: filters.schoolCode && filters.schoolCode !== "all-schools" ? roleData.name : undefined,
            Role: role.charAt(0).toUpperCase() + role.slice(1),
            Score: roleData.percentage.toFixed(1),
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    const title = `Leadership Evaluation Report - ${filters.district || "Rwanda"} - ${filters.month}${filters.sector ? ` - ${filters.sector}` : ""}${filters.schoolCode && filters.schoolCode !== "all-schools" ? ` - ${metrics[filters.schoolCode]?.headteacher?.name || "School"}` : ""}`;
    ...
    if (!filters.district) {
      ...
      const evaluationData = Object.keys(metrics).flatMap((key) =>
        ["headteacher", "dos", "dod"].map((role) => {
          ...
          return [
            filters.district,
            roleData.sector || filters.sector || "All Sectors",
            filters.schoolCode && filters.schoolCode !== "all-schools" ? roleData.name : undefined,
            role.charAt(0).toUpperCase() + role.slice(1),
            roleData.percentage.toFixed(1),
      ...
      autoTable(doc, {
        head: [
          [
            "District",
            "Sector",
            ...(filters.schoolCode && filters.schoolCode !== "all-schools" ? ["School"] : []),
            "Role",
            "Score (%)",
            "Evaluated by (%)",
            "Date",
              ...
              <div className="flex-col md:flex-row flex items-start md:items-center justify-between gap-4 md:gap-12">
                <div>
                  ...
                  <p className="opacity-90">
                    {filters.schoolCode && filters.schoolCode !== "all-schools"
                      ? metrics[filters.schoolCode]?.headteacher?.name || "School"
                      : filters.sector
                      ? filters.sector
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {overviewData.slice(0, 3).map((item) => {
                  ...
                  return (
                    <Card key={`${item.key}-${item.role}`} className="shadow-lg">
                      <CardContent className="p-6">
                        <div className="text-center">
                          ...
                          <div className="mt-4">
                            ...
                            {filters.district && (
                              <div className="text-sm text-gray-600 mt-1">
                                Evaluated by: {item.evaluationPercentage.toFixed(1)}%
                              </div>
                            )}
                          </div>
                        </div>
              ...
              <div className="grid grid-cols-1 gap-6">
                {Object.keys(metrics).flatMap((key) =>
                  ["headteacher", "dos", "dod"].map((role) => {
                    ...
                    if (!roleData || roleData.responseCount === 0) return null;
                    const roleLabels = {
                      headteacher: filters.schoolCode && filters.schoolCode !== "all-schools"
                        ? schools.find((s) => s.schoolCode === key)?.headTeacherName || "Headteacher"
                        : "Headteacher",
                      dos: filters.schoolCode && filters.schoolCode !== "all-schools"
                        ? schools.find((s) => s.schoolCode === key)?.dosName || "Deputy Head (Studies)"
                        : "Deputy Head (Studies)",
                      dod: filters.schoolCode && filters.schoolCode !== "all-schools"
                        ? schools.find((s) => s.schoolCode === key)?.dodName || "Deputy Head (Discipline)"
                        : "Deputy Head (Discipline)",
                    };
                    return (
                      <Card key={`${key}-${role}`} className="shadow-lg">
                        <CardHeader className="bg-gradient-to-r from-gray-50 to-gray-100">
                          <CardTitle className="flex items-center justify-between">
                            <span>
                              {roleLabels[role as keyof typeof roleLabels]} - {roleData.name}
                            </span>
                              ...
                              <div key={score} className="flex items-center space-x-3">
                                ...
                                <div className="flex-1 bg-gray-200 rounded-full h-2">
                                  <div
                                    ...
                                    style={{
                                      width: `${((roleData.responseCount || 0) / roleData.responseCount) * 100}%`,
                                    }}
                                  />
                                </div>
                                <span className="w-12 text-sm text-gray-600">
                                  {roleData.responseCount || 0}
                                </span>
                              </div>
                            ))}
                          </div>
              ...
              <Card className="shadow-lg">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={400}>
                    <BarChart 
                      data={overviewData} 
                      layout={filters.district && (filters.sector || (filters.schoolCode && filters.schoolCode !== "all-schools")) ? "horizontal" : "vertical"}
                    >
                      <CartesianGrid strokeDasharray="3 3" />
                      {filters.district && (filters.sector || (filters.schoolCode && filters.schoolCode !== "all-schools")) ? (
                        <>
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className="text-sm text-yellow-700">
                                ...
                                {filters.district && `, Evaluated by: ${item.evaluationPercentage.toFixed(1)}%`}
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  <thead>
                    {filters.district ? (
                      <tr className="bg-gray-100">
                        ...
                        {filters.sector || (filters.schoolCode && filters.schoolCode !== "all-schools") ? (
                  ...
                  <tbody>
                    {filters.district ? (
                      overviewData.map((item) => {
                        ...
                        return (
                          <tr key={`${item.key}-${item.role}`} className="hover:bg-gray-50">
                            <td className="p-3 border border-gray-300">{filters.district}</td>
                            {filters.sector || (filters.schoolCode && filters.schoolCode !== "all-schools") ? (
                              <td className="p-3 border border-gray-300">{item.sector || filters.sector}</td>
                            ) : null}
                            {filters.schoolCode && filters.schoolCode !== "all-schools" ? (
                            ...
                            <td className="p-3 border border-gray-300">{item.evaluationPercentage.toFixed(1)}</td>
                          </tr>
                        );
                      })
                    ) : (
